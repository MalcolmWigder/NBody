...
# Calculate initial speeds and set velocities
...
while running:
    ...
    # Compute speed and normalize for heatmap
    speed = np.linalg.norm(velocities, axis=1)
    speed_norm = (speed - speed.min()) / (speed.max() - speed.min() + 1e-8)

    # Draw particles with velocity-based heatmap
    for i in range(NUM_PARTICLES):
        # Map speed to color (slow = blue, fast = red)
        r = int(255 * speed_norm[i])
        g = 50
        b = int(255 * (1 - speed_norm[i]))
        color = (r, g, b)
        pygame.draw.circle(screen, color, (int(positions[i, 0]), int(positions[i, 1])), int(radii[i]))
